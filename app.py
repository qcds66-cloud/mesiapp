...
def load_all_data():
    food_data = [
        ...
        {"word": "スープ", "kanji": "濃湯", "meaning": "湯 / 濃湯", "ex1_kanji": "コーンの 甘みが 広がる 温かい スープを 飲む。", "ex1_kana": "こｰんのあまみがひろがるあたたかいすｰぷをのむ", "ex1_cn": "喝一碗充滿玉米甜味的溫暖濃湯。", "ex2_kanji": "温かいスープを飲みます。", "ex2_kana": "あたたかいすｰぷを飲みます", "ex2_cn": "喝熱湯。"},
        ...
        {"word": "マンゴー", "kanji": "芒果", "meaning": "芒果", "ex1_kanji": "トロピカルな 甘さが 特徴の 完熟 マンゴーを 頬張ります。", "ex1_kana": "とろぴかるなあまさがとくちょうのかんじゅくまんごｰをほうばります", "ex1_cn": "大口品嚐以熱帶甜味為特色的大熟芒果。", "ex2_kanji": "マンゴーアイスを食べます。", "ex2_kana": "まんごｰあいすをたべます", "ex2_cn": "吃芒果冰淇淋。"},
    ...
    return {
        "食物與水果 (50個)": food_data,
        "動物與魚類 (50個)": animal_data,
        "生活日常用品 (50個)": daily_data,
        "自然天氣 (35個)": nature_data,
        "學校學科 (25個)": subject_data,
        "家族稱謂 (40個)": family_data,
        "心情情緒 (30個)": emotion_data,
        "方向與位置 (30個)": direction_data
    }
...
<div class="control-panel">
    <button id="playAllBtn" class="btn btn-primary">▶️ 連續朗讀本課全單字（文字同步切換）</button>
    
    <div class="sub-btn-group">
        <button id="playWordBtn" class="btn btn-sub">🔊 僅讀單字</button>
        <button id="playEx1Btn" class="btn btn-sub">🔊 僅讀例句 1</button>
        <button id="playEx2Btn" class="btn btn-sub">🔊 僅讀例句 2</button>
    </div>

    <button id="stopBtn" class="btn btn-danger" style="width: 100%;">⏹️ 停止朗讀</button>
    <div id="statusText" class="status-text">狀態：待命</div>

    <!-- 頁碼與手動切換 -->
    <div class="nav-panel">
        <button id="prevBtn" class="btn btn-nav">⬅️ 上一單字</button>
        <span id="pageInfo" style="font-size: 18px; font-weight: bold; color: #4A403A;">1 / 1</span>
        <button id="nextBtn" class="btn btn-nav">下一單字 ➡️</button>
    </div>
</div>
...
<script>
    ...
    function highlightParticles(text) {{
        const multiParticles = ["から", "より", "まで"];
        const singleParticles = ["を", "に", "で", "が", "の", "へ", "と", "は", "も", "や", "て"];
        
        const tokens = text.split(" ");
        return tokens.map(token => {{
            if (multiParticles.includes(token) || singleParticles.includes(token)) {{
                return `<span class="particle">${{token}}</span>`;
            }}
            return token;
        }}).join(" ");
    }}
    ...
    function renderCard(index) {{
        ...
        const kanjiText = (item.kanji && item.kanji !== "-") ? ` (${{item.kanji}})` : "";
        document.getElementById('wordTitle').innerText = item.word + kanjiText;
        document.getElementById('wordMeaning').innerText = item.meaning;

        document.getElementById('ex1Kanji').innerHTML = highlightParticles(item.ex1_kanji);
        document.getElementById('ex1Kana').innerText = item.ex1_kana;
        document.getElementById('ex1Cn').innerText = item.ex1_cn;

        document.getElementById('ex2Kanji').innerHTML = highlightParticles(item.ex2_kanji);
        document.getElementById('ex2Kana').innerText = item.ex2_kana;
        document.getElementById('ex2Cn').innerText = item.ex2_cn;

        document.getElementById('pageInfo').innerText = `${{currentIndex + 1}} / ${{dataset.length}}`;
    }}

    function setStatus(text) {{
        document.getElementById('statusText').innerText = "狀態：" + text;
    }}
    ...
    // 自動連續朗讀流程（先更新畫面，停頓再朗讀）
    function startAutoPlaySequence(index) {{
        if (!isAutoPlaying || index >= dataset.length) {{
            isAutoPlaying = false;
            setStatus("全課連續朗讀完成！");
            return;
        }}
        ...
        setStatus(`正在朗讀 第 ${{index + 1}} / ${{dataset.length}} 個單字：${{item.word}}`);

        // 2. 給予 0.3 秒畫面渲染緩衝，確保視覺先更新後再發聲
        setTimeout(() => {{
            ...
            speakText(item.word, () => {{
                ...
                // 朗讀例句 1
                speakText(item.ex1_kana, () => {{
                    if (!isAutoPlaying) return;

                    // 朗讀例句 2
                    speakText(item.ex2_kana, () => {{
                        if (!isAutoPlaying) return;

                        // 進入下一個單字
                        startAutoPlaySequence(index + 1);
                    }});
                }});
            }});
        }}, 300);
    ...
    document.getElementById('playAllBtn').onclick = function() {{
        window.speechSynthesis.cancel();
        isAutoPlaying = true;
        startAutoPlaySequence(0); // 從第一個單字開始自動連續朗讀
    }};
    ...
    document.getElementById('playWordBtn').onclick = function() {{
        isAutoPlaying = false;
        speakText(dataset[currentIndex].word);
    }};

    document.getElementById('playEx1Btn').onclick = function() {{
        isAutoPlaying = false;
        speakText(dataset[currentIndex].ex1_kana);
    }};

    document.getElementById('playEx2Btn').onclick = function() {{
        isAutoPlaying = false;
        speakText(dataset[currentIndex].ex2_kana);
    }};
...
st.components.v1.html(app_component_code, height=720, scrolling=True)
